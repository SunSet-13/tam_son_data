...
def _get_conn_with_clear_error(hook, conn_id):
    try:
        return hook.get_conn()
    except psycopg2.OperationalError as exc:
        conn = hook.get_connection(conn_id)
        raise RuntimeError(
            f'Cannot connect to Airflow connection "{conn_id}" '
            f'(host={conn.host}, database={conn.schema}, user={conn.login}, port={conn.port}). '
            f'Check the host, username, password, security group, and database name. '
...
def postgres_to_ec2_transfer():
    ...
    def transfer_all_public_tables():
        ...
        local_conn = _get_conn_with_clear_error(local_hook, 'postgres')
        ec2_conn = None
        ...
        try:
            ...
            print(f'Postgres Docker source scan: found {len(source_tables)} public tables')

            ec2_conn = _get_conn_with_clear_error(ec2_hook, 'postgres_ec2')
            ...
            print(
                'Postgres Docker source scan: '
                f'found {len(source_tables)} public tables, '
                f'{len(tables)} matched EC2 target tables, '
                f'{len(skipped_tables)} skipped'
            )
            if skipped_tables:
                print(f'Skipped tables not present on EC2: {skipped_tables}')
            print(f'Table sync order: {ordered_tables}')
            ...
            for table in ordered_tables:
                ...
                source_row_count = _count_rows(local_cur, table)
                ...
                insert_sql = f'{insert_sql} RETURNING 1'
                ...
                inserted_rows = 0
                chunk_size = 5000
                print(f'{table}: source has {source_row_count} rows in Postgres Docker')
                ...
                for chunk in pd.read_sql_query(
                    ...
                    print(
                        f'  read chunk {len(records)} rows '
                        f'(read total {total_rows}, inserted total {inserted_rows})'
                    )
                ...
                if inserted_rows == 0:
                    print(f'{table}: no new rows inserted')
                else:
                    print(f'{table}: inserted {inserted_rows} rows into EC2')
            ...
            updated_tables = sum(1 for stat in table_stats if stat['inserted_rows'] > 0)
            total_inserted_rows = sum(stat['inserted_rows'] for stat in table_stats)
            print('\n=== Postgres Docker to EC2 sync summary ===')
            print(f'Source tables found in Postgres Docker: {len(source_tables)}')
            print(f'Tables checked for sync: {len(ordered_tables)}')
            print(f'Tables updated on EC2: {updated_tables}')
            print(f'Total rows inserted on EC2: {total_inserted_rows}')
            for stat in table_stats:
                print(
                    f"{stat['table']}: source_rows={stat['source_rows']}, "
                    f"read_rows={stat['read_rows']}, "
                    f"inserted_rows={stat['inserted_rows']}"
                )

            return {
                'source_tables_found': len(source_tables),
                'tables_checked': len(ordered_tables),
