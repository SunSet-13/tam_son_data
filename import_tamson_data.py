...
# Thư mục chứa CSV files
CSV_DIR = 'tamson_sales_data'

# Thứ tự import - dependencies trước, dependents sau
TABLE_MAPPING = {
...
def import_csv_to_db(csv_file, table_name):
    ...
    try:
        ...
        if df.empty:
            print(f"⏭️  {table_name}: Không có dữ liệu trong file")
            return True
        
        # Làm sạch tên cột - lowercase
        df.columns = [col.lower().replace(' ', '_') for col in df.columns]
        
        # Replace NaN với None (PostgreSQL NULL)
        df = df.where(pd.notnull(df), None)
        
        # Lọc chỉ các column tồn tại trong database
        conn = psycopg2.connect(**DB_CONFIG)
        cursor = conn.cursor()
        
        # Lấy các column của bảng
        cursor.execute(f"""
            SELECT column_name 
        ...
        # Giữ chỉ các column trong DataFrame mà tồn tại trong DB
        df_columns_to_keep = [col for col in df.columns if col in db_columns]
        df = df[df_columns_to_keep]
        
        # Disable constraints
        cursor.execute("SET CONSTRAINTS ALL DEFERRED")
        
        # Import bằng pandas to_sql
        ...
        print(f"❌ Lỗi import {table_name}: {str(e)[:100]}")
        return False

def main():
    """Hàm chính"""
    print("=" * 60)
    print("Import dữ liệu Tam Sơn vào database")
    print("=" * 60)
    ...
    print("=" * 60)
    print(f"Hoàn tất: {success_count}/{total_count} bảng")
    print("=" * 60)
